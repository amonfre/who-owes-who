		...
		if (ordered_networths[0] + ordered_networths[-1]) < 0:
			...
			for l in range(1,len(ordered_networths)-1):
				...
				if ordered_networths[l] + ordered_networths[-1] == 0:
					make_payment(new_graph, ordered_networths, ordered_networths_indexes, l, -1, ordered_networths[-1])
					break
					...
					make_payment(new_graph, ordered_networths, ordered_networths_indexes, 0, -1, ordered_networths[-1])
					break
		...
		#otherwise, simply match largest amount owed to largest amount required
			...
			for l in reversed(range(1, len(ordered_networths)-1)):
				...
				if ordered_networths[0] + ordered_networths[l] == 0:
					make_payment(new_graph, ordered_networths, ordered_networths_indexes, 0, l, ordered_networths[l])
					break
					...
					make_payment(new_graph, ordered_networths, ordered_networths_indexes, 0, -1, -(ordered_networths[0]))
					break
				#otherwise, if we've gone through all options, match largest to largest
				else:
					if l == 1:
